...
<table width="100%" border="0" cellspacing="0" cellpadding="0"	style="margin-top: 10px;">
   ...
   %>
	...
	<tr class="ct_list_pop">
		<td align="center"><%=no--%>
			<a href="/getPurchase.do?tranNo=<%= pcvo.getTranNo() %>"><%= pcvo.getTranNo() %></a>
		</td>
		<td></td>
		<td align="left">
		...
		<td></td>
		<td align="left">
			<%if(pcvo.getTranCode().trim().equals("1")) {%>
				현재 구매완료 상태 입니다. &nbsp;<a href="/updateTranCode.do?prodNo=<%=pcvo.getPurchaseProd().getProdNo()%>&tranCode=3">물건도착</a>
			<%} else if(pcvo.getTranCode().trim().equals("2")) {%>
				현재 배송중 상태 입니다.
			<%} else {%>
				현재 배송완료 상태 입니다.
			<%} %>
